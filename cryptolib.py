...
# returns 128-bit padded version of dbytes
...
def encrypt(plain, alg, key, iv):
    ...
    key = verifyKey(key, key_size)
    # pad key based on algorithm provided
    padded_key = pad(key.encode(), key_size)

    # encrypt
    cipher = Cipher(algorithms.AES(padded_key), modes.CBC(iv), backend)
